...
# Note: for reading histcase, see the initilize_mcmc script

remwet_Mkm2_stack <- readRDS(paste0('../output/results/wetloss/grid/remwet_tot_stack_0.5deg_serial_', params_type,'.rds'))
...
for (i in seq(1, nrow(histcases))) {
  ...
  wet_categ  <- histcases@data[i,"wet_categ"]
  
  
  
  # select single histcase polygon
  #histcases_poly <- histcases_poly[!is.na(histcases_poly@data$src_id),]
  t_cases_poly <- histcases[histcases$src_id==t_src_id & histcases$rec_id==t_rec_id,]
  ...
  hyde_yr_start <- years[which.min(abs(years - t_yr_start))]
  hyde_yr_end   <- years[which.min(abs(years - t_yr_end))]
...
write.csv(output_df, "../output/results/histcase_mappedwetloss_extracted_v4_serial.csv")


# delete objects 
# rm(output_df, years, remwet_Mkm2_stack)
...
# /----------------------------------------------------------------------------#
#/ Prep extracted mapped wetloss for rejoining to poly & plot            -------


f<-"../output/results/histcase_mappedwetloss_extracted_v4_serial.csv"
...
cs_extract <- read.csv(f, stringsAsFactors = F) %>%
  ...
  group_by(src_id, rec_id, rast_source) %>%
  ...
  group_by(src_id, rec_id, country, hcase_start_year, 
           hyde_start_yr, hcase_end_year, hyde_end_yr, remwet_start, remwet_end) %>%
...
# /----------------------------------------------------------------------------#
#/ Rejoin extraction to the polygons
...
# Save polygons w/ data as RDS again
f = "../data/hist_records/davidson_sites_gis/davidson_sites_wdata_manmod_wmaploss_v2_serial.rds"
saveRDS(histcases_wmaploss, f)
...
# n <- "./output/results/hyde_resampled/wetloss_distrib_05deg.nc"
# nc <- nc_open(n)

# get raster stacks of remaining wetland
# remwet_Mkm2_stack <- readRDS('./output/results/wetloss/grid/remwet_Mkm2_stack_0.5deg.rds')
# remwet_Mkm2_stack <- readRDS('../output/results/wetloss/grid/remwet_tot_stack_0.5deg_serial.rds')