...
# create record IDs
drained_ag <-   drained %>% dplyr::filter(type == "cropland")
...
artdrain_ag_nat <- sel_countries_in_shp(drained_ag, countries, "iso_a3", "adm0_a3", "rec_id")


# join the data table to shapefile, using rec_id as key
artdrain_ag_nat@data <- merge(artdrain_ag_nat@data, as.data.frame(drained), 
...
saveRDS(artdrain_ag_nat, "./output/results/artif_drainage/artdrain_ag_nat.rds")
