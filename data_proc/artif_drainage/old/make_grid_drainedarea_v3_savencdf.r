...
for (y in 1:31){
  ...
  # /--------------------------------------------------------------------------#
  #/    Total wetloss                                                     ------
  tot_loss <- stack(cropland_drain_distrib, 
  ...
  tot_loss <- sum(tot_loss, na.rm=TRUE)
  
  # /---------------------------------------------------------------------
  #/  SAVE OUTPUTS!!!!
  
  # make list of string variable names. 
  ...
  for ( r in dist_rast_list){
    ...
    # save individual drivers of loss
  ...
  # Save total loss grid to NetCDF
  ncvar_put(outncdf,   # the output ncdf
            'tot_wetloss',   # the variable string name
            values(flip(get('tot_loss'), 'y')), #get(v),   # the raster to write
            start=c(1,1,y),     # the index start
            count=c(nx,ny,1))   # the index count/size
...
saveRDS(s_out, './output/results/wetloss/grid/wetloss_bydriver_stack_0.5deg.rds')
