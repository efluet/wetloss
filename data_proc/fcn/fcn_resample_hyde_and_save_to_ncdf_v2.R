...
# Create function
# Arguments:   
#   - X & Y resolution of wetland grid, 
#   - start & end year 
save_resampled_hyde_data <- function(wet_res_x, wet_res_y, start_year, end_year){
  ...
  # keep only AD years
  year_ls <- year_ls[grep(pattern='AD', x=year_ls)]
  year_ls <- sub(pattern= "/", "", x=year_ls)
  ...
  ncfname <- paste0(o, "hyde32_", wet_res_y,"_allvars.nc")
  ...
  var_ls_symbols <- list(cropland, ir_rice, cropland, grazing, 
                         pasture, rangeland, conv_rangeland, 
                         rf_rice, ir_rice, rf_norice, ir_norice, 
                         tot_rice, tot_rainfed, tot_irri)
