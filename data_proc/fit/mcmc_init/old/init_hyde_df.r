...
peatland   <- left_join(maxlncr_df_xy, peatland, by=c('x','y'))[c(3:33, 33, 33)]
peatextr   <- peatland
...
forestry <- left_join(maxlncr_df_xy, forestry, by=c('x','y'))[c(3:33, 33, 33)]  #[3:33]
...
ir_rice    <- left_join(maxlncr_df_xy, ir_rice, by=c('x','y'))[c(3:33, 33, 33)]  #[3:33]

urban      <- raster2df(stack(h, varname='uopp_'))
urban      <- left_join(maxlncr_df_xy, urban, by=c('x','y'))[c(3:33, 33, 33)]  #[3:33]

pasture    <- raster2df(stack(h, varname='pasture'))
pasture   <- left_join(maxlncr_df_xy, pasture, by=c('x','y'))[c(3:33, 33, 33)]  #[3:33]

cropland   <- raster2df(stack(h, varname='cropland'))
cropland   <- left_join(maxlncr_df_xy, cropland, by=c('x','y'))[c(3:33, 33, 33)]  #[3:33]
