...
wetloss_at1700 <-  wetloss_all %>% filter(year == 1700) %>% dplyr::select(-one_of("year"))
names(wetloss_at1700) <- c("name", "overlap",  "tot_wet_Mkm2_in1700", "tot_remwet_Mkm2_in1700")


wetloss_pre1700 <- wetloss_all %>% 
                   filter(year <= 1700, name=="trace21_129.cdf") %>%
                   left_join(., wetloss_at1700, by=c("name", "overlap")) %>%
                   mutate(remwet_percfrom1700 = tot_remwet_Mkm2/tot_remwet_Mkm2_in1700 *100) %>%
                   mutate(natwet_percfrom1700 = tot_wet_Mkm2/tot_remwet_Mkm2_in1700 *100)
