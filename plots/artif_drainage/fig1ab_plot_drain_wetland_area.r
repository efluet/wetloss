...
#  Read in data
die <- read.csv('../output/results/artif_drainage/drained_wetcult_gridded_sum_fullserial.csv') %>%
...
barplot_lutype <- 
  ...
  geom_area(aes(x=year, y=cumul_drain_km2/1000, fill=die$lu_type_fac), color='white', size=0.05)+
  ...
  scale_fill_manual(#labels = driver_names,
                    values =
                      c('Cropland' = '#ff5b4f',  # Cropland
                        'Wetcult'  = '#507dc7',  # Wetcultiv - blue
                        'Forest'   = '#8df0a9',  # Forestry
  ...
  ylab(expression(paste("Drained or converted area (10"^{3},' km'^{2}," )" ))) +
  xlab("") +
...
wetarea_lineplot <- 
  ...
  scale_y_continuous(expand=c(0,0), limits=c(10, 20), labels = seq(10, 20, 2), breaks= seq(10,20,2)) +
