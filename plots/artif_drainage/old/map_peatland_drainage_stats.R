...
# read drainage shp
artdrain_peatland_natpoly <- readRDS("./output/results/artif_drainage/artdrain_nat_poly_peatland.rds")



# Fortify & reproject polygons to ggplot-mappable df 
artdrain_peatland_natpoly_df <- prep_poly_into_robin_map_wdata(artdrain_peatland_natpoly)
...
map <- 
  ...
  # add data countries
  geom_polygon(data=artdrain_peatland_natpoly_df, 
  ...
  scale_fill_distiller(type="seq", direction=1, palette = 7) +
...
ggsave('./output/figures/artif_drainage/map/perc_drained_peatland.png',  
