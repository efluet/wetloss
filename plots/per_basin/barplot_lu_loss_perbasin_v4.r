...
# # /----------------------------------------------------------------------------#
# #/   Get wetloss area
# remwet_Mkm2_stack_best <- readRDS('../output/results/wetloss/grid/remwet_tot_stack_0.5deg_serial_best.rds')
# remwet_Mkm2_stack_low <- readRDS('../output/results/wetloss/grid/remwet_tot_stack_0.5deg_serial_low.rds')
# remwet_Mkm2_stack_high <- readRDS('../output/results/wetloss/grid/remwet_tot_stack_0.5deg_serial_high.rds')
# 
# get_wetloss_perc <- function(endyear){
# 
#   beststack <- remwet_Mkm2_stack_best[[grep(pattern=endyear, names(remwet_Mkm2_stack_best))]]
#   lowstack <- remwet_Mkm2_stack_low[[grep(pattern=endyear, names(remwet_Mkm2_stack_low))]]
#   highstack <- remwet_Mkm2_stack_high[[grep(pattern=endyear, names(remwet_Mkm2_stack_high))]]
# 
#   
#   z_best <- as.data.frame(zonal(beststack, rivbasin, 'sum'))
#   z_low  <- as.data.frame(zonal(lowstack, rivbasin, 'sum'))
#   z_high <- as.data.frame(zonal(highstack, rivbasin, 'sum'))
# 
#   z <- bind_cols(z_best, z_low, z_high) %>% 
#        select(-zone1, -zone2)
#   names(z) <- c('zone','best','low','high')
#   
# 
#   z <- z %>% 
